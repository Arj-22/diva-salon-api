...
import { FormSubmissionSchema, } from "../../utils/schemas/FormSubmissionSchema.js";
...
const supabase = SUPABASE_URL && SUPABASE_ANON_KEY
    ...
    : null;
formSubmissions.post("/", hcaptchaVerify({ bodyField: "hcaptcha_token" }), async (c) => {
    if (!supabase)
    ...
    if (submission.email) {
        const { data, error } = await supabase
            ...
            .maybeSingle();
        if (error) {
            return c.json({ error: "Failed to lookup client by email", details: error.message }, 500);
        }
        if (data)
            clientRow = data;
    }

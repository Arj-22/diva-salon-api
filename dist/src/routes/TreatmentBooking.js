...
const supabase = SUPABASE_URL && SUPABASE_ANON_KEY
    ...
    : null;
treatmentBookings.get("/", cacheResponse({
    key: (c) => {
    ...
    const { data, error, count } = await supabase
        ...
        .select(`*, Treatment (EposNowTreatment(Name, SalePriceIncTax))
        `, { count: "exact" })
        .eq(status ? "status" : "", status || "")
        .order("created_at", { ascending: false })
        .range(start, end);
    if (error) {
        return c.json({ error: "Failed to fetch treatment bookings", details: error.message }, 500);
    }
...
treatmentBookings.get("/byBookingId/:bookingId{[0-9]+}", cacheResponse({
    key: (c) => {
        return buildCacheKey("treatmentBookings", {
...
treatmentBookings.get("/byId/:Id{[0-9]+}", cacheResponse({
    key: (c) => {
        return buildCacheKey("treatmentBookings", {
            Id: c.req.param("Id"),
        });
...
treatmentBookings.patch("updateTreatmentBooking/:id{[0-9]+}", validateTreatmentBooking(), async (c) => {
    if (!supabase)
        return c.json({ error: "Supabase not configured" }, 500);
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error) {
        return c.json({ error: "Failed to update treatment booking", details: error.message }, 500);
    }

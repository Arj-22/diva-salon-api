...
eposNowCategories.get("/", cacheResponse({ key: "eposNowCategories:all", ttlSeconds: 300 }), async (c) => {
    if (!supabase)
...
eposNowCategories.post("/insertNewCategories", async (c) => {
    ...
    const categoriesToInsert = payloads.filter((cat) => !categoryIdsList.includes(cat.CategoryIdEpos));
    ...
    const { data: upserted, error: upsertError } = await supabase
        ...
        .select();
    if (upsertError) {
        return c.json({ error: "Failed to upsert categories", details: upsertError.message }, 500);
    }
    return c.json({

...
eposNowTreatments.post("insertTreatmentsByEposCategory", async (c) => {
    ...
    const text = await res.text();
    const productsForCategory = JSON.parse(text).filter((p) => p.CategoryId === eposCategoryId);
    ...
    const payloads = productsForCategory.map((t) => ({
        EposNowId: t.Id,
        Name: t.Name,
    ...
    const { data: upserted, error: upsertError } = await supabase
        ...
        .select();
    if (upsertError) {
        return c.json({ error: "Failed to upsert treatments", details: upsertError.message }, 500);
    }
    return c.json({
...
eposNowTreatments.post("/insertNewTreatments", async (c) => {
    ...
    // Build payloads for upsert and use Name as the conflict target
    const payloads = treatments.map((t) => ({
        EposNowId: t.Id,
        Name: t.Name,
        Description: t.Description ?? null,
        SalePriceIncTax: t.IsSalePriceIncTax ? t.SalePrice : null,

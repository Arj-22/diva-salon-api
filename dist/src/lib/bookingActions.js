...
const supabase = SUPABASE_URL && SUPABASE_ANON_KEY
    ...
    : null;
export async function createBookingRecord(c, payload) {
    if (!supabase) {
    ...
    if (conflictingBooking) {
        return c.json({ error: "This appointment start time is already booked" }, 409);
    }
    ...
    if (treatmentError || !treatmentRow) {
        return c.json({ error: "Failed to load treatment duration" }, treatmentError ? 500 : 404);
    }
    const appointmentStart = new Date(bookingData.appointmentStartTime);
    const appointmentEndTime = new Date(appointmentStart.getTime() + treatmentRow.durationInMinutes * 60000).toISOString();
    ...
    const { data: bookingRow, error: bookingError } = await supabase
        ...
        .select("id, message, clientId, treatmentId, staffId, appointmentStartTime, appointmentEndTime, Treatment (*, EposNowTreatment(Name, SalePriceIncTax))")
        .single();
    ...
    // @ts-ignore
    const treatment = bookingRow.Treatment.EposNowTreatment;
    try {
        await sendEmail({
